        ...
        if (count($aktivitas)>0) {
          ...
          foreach ($aktivitas as $kd => $vd) {
            ...
            if (count($soal)>0) {
              ...
              $this->response(array('status' => 200, 'message' => 'Data Soal Belum Diisi', 'data' => null));
            }
          }
          ...
          if ($list_aktivitas) {
            ...
            $this->response(array('status' => 200, 'message' => 'Data Grading masih kosong!', 'data' => null));
          }
        } else {
    ...
    if(!empty($id_pelatihan) && !empty($id_user) && !empty($id_kelas)){
      ...
      array(
        ...
        'hasil_test_terakhir'=>array(
          'kondisi_fisik' => ($kondisi?$return_fisik = $this->GeneralApiModel->getWhereMaster(array('id' => $kondisi->kondisi_fisik),'masterdata_grading_status_kesehatan')->row()->nama:null),
          'kondisi_mental' => ($kondisi?$return_fisik = $this->GeneralApiModel->getWhereMaster(array('id' => $kondisi->kondisi_mental),'masterdata_grading_status_kesehatan')->row()->nama:null),
...
function countdown_next($date){
  ...
  return array('hari'=>$jam==24 && $hari!=14?$hari+1:$hari, 'jam'=>$jam==24?0:$jam);
}
