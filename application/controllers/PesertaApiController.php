...
class PesertaApiController extends REST_Controller
{
  ...
  function homePeserta_post(){
    ...
    if(!empty($id_kelas) && !empty($id_user)){
      ...
      $result = array(
        'kondisi_fisik' => $return_fisik = $this->GeneralApiModel->getWhereMaster(array('id' => $kondisi->kondisi_fisik),'masterdata_grading_status_kesehatan')->result()[0]->nama,
        'kondisi_mental' => $return_fisik = $this->GeneralApiModel->getWhereMaster(array('id' => $kondisi->kondisi_mental),'masterdata_grading_status_kesehatan')->result()[0]->nama,
        'skrining_terakhir'=> array(
          'id'=>$kondisi->id_skrining,
          'is_sudah_skrining'=>($this->date_diff($kondisi->cdate)<=14?true:false)
        ),
        'presensi_terakhir' => array(
          'waktu'=>$presensi->cdate,
          'is_sudah_presensi'=>($this->date_diff($presensi->cdate)==0?true:false)
        ),
        'laporan_harian_terakhir' => array(
          'id'=> $aktivitas->id_aktivitas,
          'waktu'=>$aktivitas->cdate,
          'is_sudah_isi_laporan'=>($this->date_diff($aktivitas->cdate)==0?true:false)
        ),
        'list_materi' => $materi,
